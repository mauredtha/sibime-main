...
class CoursesController extends Controller
{
    ...
    public function showMapelGuruKode($level, $kode){

      $courses = Courses::where('kode_guru', '=', $kode)->orWhere('kode_guru2', '=', $kode)->orWhere('kode_guru3', '=', $kode)->orWhere('kode_guru4', '=', $kode)->orWhere('kode_guru5', '=', $kode)->get();

      return view('course.Mapel',compact('courses','level'));
    }
    ...
    //kepsek
    public function showMapel($tahun, $level){
        $tahun = str_replace('_', '/', $tahun);
        $courses['Mapel'] = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->orderBy('name','asc')->get();

        $courses['Kelas'] = Classes::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->orderBy('name','asc')->get();

        $courses['LabelKelas'] = array();
        $courses['DataSiswa'] = array();
        $courses['DataGuru'] = array();
        foreach ($courses['Kelas'] as $key => $value) {
          $courses['LabelKelas'][$key] = $value->name;

          $siswa = User::where(['role' => 'Siswa', 'class_id' => $value->id])->count();
          $courses['DataSiswa'][$key] = $siswa;

          $guru1 = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->count('kode_guru');
          $guru2 = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->count('kode_guru2');
          $guru3 = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->count('kode_guru3');
          $guru4 = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->count('kode_guru4');
          $guru5 = Courses::where(['tahun_ajaran'=>$tahun, 'level'=>$level])->count('kode_guru5');


          $courses['DataGuru'][$key] = $guru1 + $guru2 + $guru3 + $guru4 + $guru5;
    ...
    public function showKomponenLevel($level, $course_id)
    {
        $data['course_info'] = Courses::where(['id'=>$course_id])->first();

        //dd($data['course_info']);

        $compactData=array('data','level');
        return View::make('course.KomponenLevel', compact($compactData));
    }
}
